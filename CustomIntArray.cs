...
public class _ArrayElement
{
    int CurrentElement = 0;
    _ArrayElement? NextElement = null;
    public _ArrayElement(int element = 0)
    {
    ...
    {
        if (size <= 1) return;
        NextElement = new _ArrayElement();
        NextElement.Generate(size - 1);
    }
    private int SizeRecalculate()
    {
        if (NextElement == null) return 1;
        return NextElement.SizeRecalculate() + 1;
    }
}
public class CustomIntArray : _ArrayElement
{
    int Size { get; set; }
    ...
    {
        if (rhs.GetSize() != Size) return false;
        for (int i = 0; i < Size; i++)
        {
